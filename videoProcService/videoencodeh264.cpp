...
//}
            ...
            if(pDataManager->bIsStartTracking == true)
            {
                if(pDataManager->bIsResetTracking == true)
                {
                    if(pDataManager->videoSrc == 0x01)
                    {
                        if(pDataManager->dayNightReverse == 0x55)
                        {
                        ...
                        else if(pDataManager->dayNightReverse == 0XAA)
                        {
                            trackAlgorithm.MatchAndTrackInit (pDataManager->recvTargetRect,capFrame,pDataManager->featureInfrared,15,2);

                            trackAlgorithm.ReadCalibrateFile (2, pDataManager->electronicZoom);
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    //qDebug()<<pDataManager->yawAngle_target<<pDataManager->pitchAngle_target;
                    ...
                    //qDebug("yaw_target = %f , pitchAngle_target = %f",pDataManager->yawAngle_target,pDataManager->pitchAngle_target);
                    //imshow("ssss",capFrame);
                    //waitKey (1);
                    if(pDataManager->assistFlag == 0x01)//open
                    {
                        ...
                        if(pDataManager->videoSrc == 0x01)
                        {
                            trackAlgorithm.Angle2Point (pDataManager->yawAngle_predict,
                                                        pDataManager->pitchAngle_predict, pointCross, 1);
                        }
                        else if(pDataManager->videoSrc == 0x02)
                        {
                            trackAlgorithm.Angle2Point (pDataManager->yawAngle_predict,
                                                        pDataManager->pitchAngle_predict, pointCross, 2);
                        }

                        //line (capFrame, Point(pointCross.x,pointCross.y-30),
                             // Point(pointCross.x,pointCross.y+30), Scalar(0,0,255), 2, 8, 0);
                        //qDebug("Point(pointCross.x,pointCross.y-30) = %d,%d",pointCross.x,pointCross.y-30);
                        //qDebug("Point(pointCross.x,pointCross.y+30) = %d,%d",pointCross.x,pointCross.y+30);
                        //line (capFrame, Point(pointCross.x-30,pointCross.y),
                              //Point(pointCross.x+30,pointCross.y), Scalar(0,0,255), 2, 8, 0);
                        ...
                        static Scalar colorTargetTag(250,250,250);
                        {
                            ...
                            line(capFrame,Point(x-40,y) , Point(x-20,y) ,colorTargetTag , 1 ,8 ,0);
                            line(capFrame,Point(x+20,y) , Point(x+40,y) ,colorTargetTag , 1 ,8 ,0);

                            line(capFrame,Point(x,y-40) , Point(x,y-20) ,colorTargetTag , 1 ,8 ,0);
                            line(capFrame,Point(x,y+20) , Point(x,y+40) ,colorTargetTag , 1 ,8 ,0);

                            circle (capFrame,Point(x,y) , 20 ,colorTargetTag , 1 ,LINE_AA ,0);
              ...
              if(pDataManager->assistFlag == 0x01)
              {
                ...
                {
                    imgCenterX = (int)pDataManager->cameraMatrix[pDataManager->focusZoom].at<double>(0,2);//capFrame.cols / 2;
                    imgCenterY = (int)pDataManager->cameraMatrix[pDataManager->focusZoom].at<double>(1,2);//capFrame.rows / 2;
                }
                line(capFrame,Point(imgCenterX-40,imgCenterY) , Point(imgCenterX+40,imgCenterY) ,Scalar(0,128,0), 2 ,8 ,0);
                line(capFrame,Point(imgCenterX,imgCenterY-40) , Point(imgCenterX,imgCenterY+40) ,Scalar(0,128,0) , 2 ,8 ,0);
             }
            //-----------end----------
